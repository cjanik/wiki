...
#        if self.username.by_name()
#    
#        checkusername = User.all()
#        checkusername.filter("name", self.username)
#        result = checkusername.get()
#        if result == None:
#            hashpassword = make_pw_hash(self.username, self.password)
#            login = User(name = self.username, pw_hash = hashpassword, email=self.email)
#            login.put()
#            self.response.headers.add_header('Set-Cookie', 'user_name=%s; Path=/' % str(self.username))
#            self.redirect('/')
#        else:
#            params['error_username'] = "That user name is in use, please choose another."
#            self.render('signup-form.html',**params)
#
...
class Wiki(db.Model):
    ...
    @staticmethod
    def parent_key(path):
        return db.Key.from_path(path, 'wookie')
...
class WikiPage(WikiHandler):
    def get(self, path):
        ...
        if v:
            if v.isdigit():
                p = Wiki.by_id( int(v), path)
            
            if not p:
                return self.notFound()
        else:
            p = Wiki.by_path(path).get()
...
class EditPage(WikiHandler):
    def get(self, path):
        ...
        self.render("editWiki.html", path = path, page = p)
    ...
    def post(self, path):
        ...
        ov = Wiki.by_path(path).get()

        if not (ov or content):
            return

        elif not ov or ov.content != content:
            p = Wiki(parent = Wiki.parent_key(path), content = content, author = self.user.name)
            p.put()
